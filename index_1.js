...
const WEB_SERVER_HOME = 'C:\\BEJ\\codelap_allola\\html';
...
app.get('/boardMain', async (req, res) => {
    ...
    try {
        ...
        if (req.query.sort === 'views_desc') {
            orderByClause = 'order by b.boarder_code DESC'; // 조회수 내림차순, 최신순
        }
        ...
        if (req.query.searchType && req.query.searchInput) {
            ...
            if (searchType === 'title') {
                ...
                searchCondition = ` AND u.boarder_code LIKE '%${searchInput}%'`;
            }
        }


        // if() 다음 블록에 들어가는 조건: true, truesy (false가 아닌 모든 값)
        // if() 다음 블록이 수행되지 않는 조건:false, falsy(0,null,NaN)
        // WHERE 1=1 있어도 없어도 그만이지만 이게 없고 input내용이 없다면 필터가 걸리지 않기때문에
        // 그걸 대비하기 위해 값을 넣어둔다.
        const sql_query = `SELECT
             ...
             FROM (
                      SELECT
                          b.boarder_code, b.title, u.nickname AS author, b.created_at, b.views, b.likes,
